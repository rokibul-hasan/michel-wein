...
function get_category() {
	...
	$kategorien = array("rotwein","weisswein","andere");

	if(!isset($_GET['kategorie'])) {
		$kategorie = "rotwein";
	}
	else {	
		if(in_array($_GET['kategorie'], $kategorien)) {
...
function get_products() {
	...
	$sql = "SELECT artikelnummer, produkt, jahrgang, qualitaet, geschmacksrichtung, preis_liter, gebinde_halb, gebinde_dreiviertel, gebinde_liter FROM `produkte` WHERE `kategorie` = ?";
	$stmt = $db -> prepare($sql);
	$stmt -> bind_param("s", $kategorie);
	$stmt -> execute();
	$stmt -> bind_result($artikelnummer, $produkt, $jahrgang, $qualitaet, $geschmacksrichtung, $preis_liter, $gebinde_halb, $gebinde_dreiviertel, $gebinde_liter);
...
function create_order_form($artikelnummer) {

    $return="<form>Bestellen: <select name=\"".$artikelnummer."\">";

    for($i=0;$i<61;$i++) {
        if(isset($_SESSION['articles'][$artikelnummer]) and $i == $_SESSION['articles'][$artikelnummer]) {
...
<ul id="slides">
<li><img src="../weinpersoenlichkeiten/images/praesente_03.jpg" alt="Weine" width="702" height="368" /></li>
  ...
  <!-- the tabs -->
      ...
      <li><a href="basket.php">Warenkorb</a></li>
	  </ul>
 
<!-- tab "panes" -->
<div class="panes">

            <!-- Nachricht, wenn etwas in den Shop gelegt oder gelöscht wird -->
			...
			?>
                ...
                <a href="weinliste_detail.php?artikel=<? echo $v['artikelnummer']; ?>" rel="#overlay" style="text-decoration:none">
                    ...
                    <ul class="weinbestellen">
                    <li><!-- Bestelldropdown -->
                    <?
                        create_order_form($v['artikelnummer']);
                    ?>
                   </li>
                    </ul>
		        </div>
